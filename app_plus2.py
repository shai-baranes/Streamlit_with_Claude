...
# ── Date range filter ────────────────────────
# TUTORIAL NOTE:
# We derive a sorted list of unique dates from the filtered df and present
# them as selectbox dropdowns — compatible with Streamlit 1.12.
# st.date_input exists in 1.12 but returns a datetime.date which requires
# extra casting; selectboxes over existing dates are simpler and error-free.

_dates_available = sorted(df["Date"].dt.date.unique().tolist())

dt_col1, dt_col2 = st.columns(2)
with dt_col1:
    tbl_start = st.selectbox(
        "📅 Table Start Date",
        options=_dates_available,
        index=0,                          # default: earliest date
        key="tbl_start",
    )
with dt_col2:
    # Default end index: last date
    _default_end_idx = len(_dates_available) - 1
    tbl_end = st.selectbox(
        "📅 Table End Date",
        options=_dates_available,
        index=_default_end_idx,           # default: latest date
        key="tbl_end",
    )
...
if tbl_start > tbl_end:
    ...
    df_table = df.iloc[0:0]              # empty df to avoid downstream errors
else:
    df_table = df[
        (df["Date"].dt.date >= tbl_start) &
        (df["Date"].dt.date <= tbl_end)
    ]
    st.caption(
        f"Showing rows from **{tbl_start}** to **{tbl_end}** "
        f"— {len(df_table)} matching records"
        # f"— {len(df_table):,} matching records"
    )
...
if display_cols and not df_table.empty:
...
# ── Download button — respects date range filter ──
st.download_button(
